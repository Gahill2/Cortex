...
interface Props {
  ...
  /** Inside unified toolbar — no outer pill chrome */
  embedded?: boolean;
}
...
export function CanvasToolbar({
  ...
  const addMenuPanel = pickingWidgetKey ? (
    ...
    <>
          {onOpenWidgetLibrary ? (
            <div className="canvas-add-menu__section">
              <button
                type="button"
                className="canvas-add-menu__item canvas-add-menu__item--library"
                onClick={() => {
                  onOpenWidgetLibrary();
                  setShowAddMenu(false);
                }}
  ...
  const toolbarControls = (
    <div
      className={`canvas-toolbar${showDashboardChrome ? " canvas-toolbar--with-dashboard" : ""}${embedded ? " canvas-toolbar--embedded" : ""}${menuOpen ? " canvas-toolbar--menu-open" : ""}`}
    >
      {showDashboardChrome && !embedded && (
        <>
      ...
      {showDashboardChrome && embedded && (
        <>
          <span className="canvas-toolbar__inline-meta">
            {widgetCount} widget{widgetCount === 1 ? "" : "s"}
          </span>
          <div className="canvas-toolbar__divider" />
        </>
      ...
      {showEmbedModal && (
        <CanvasModalPortal open={showEmbedModal} onClose={() => setShowEmbedModal(false)}>
          <EmbedModal
  ...
  return (
    <>
      <div className={`canvas-toolbar-stack${menuOpen ? " canvas-toolbar-stack--menu-open" : ""}`}>
        {toolbarControls}
      </div>
      <CanvasMenuPortal
        open={showBgMenu}
        ...
        menuWidth={260}
        className="canvas-add-menu canvas-bg-menu canvas-add-menu--portaled"
      >
        {bgMenuPanel}
      </CanvasMenuPortal>
      <CanvasMenuPortal
        ...
        menuWidth={addMenuWidth}
        className={`canvas-add-menu canvas-add-menu--portaled${pickingWidgetKey ? " canvas-add-menu--variant" : ""}`}
      >
        {addMenuPanel}
      </CanvasMenuPortal>
    </>
